...
| 8 | 0.26 | 0.68 | 0.28 |
| 16 | 0.15 | 0.94 | 0.16 |
| 512 | **0.004** | **219.8** | **0.83** |

### 生成样本统计（投影回 2D 后的半径）

| D | x-pred | ε-pred | v-pred | GT |
|---|--------|--------|--------|-----|
| 2 | 1.04±0.20 | 1.00±0.19 | 0.99±0.17 | 1.0 |
| 8 | 1.07±0.19 | 0.70±0.39 | 0.99±0.21 | 1.0 |
| 16 | 1.09±0.18 | 0.95±0.82 | 0.94±0.29 | 1.0 |
| 512 | **1.09±0.19** | **1279±671** | **1.25±0.66** | 1.0 |

### 可视化结果

#### KDE 密度图

![KDE Visualization](toy_results/figure_2_kde.png)
...
2. **D=8**: ε-pred 开始退化（半径偏小 0.70，方差增大）
3. **D=16**: ε-pred 明显退化；v-pred 略有偏差
4. **D=512**: x-pred 仍完美（半径 1.09），ε-pred 彻底崩溃（半径 1279），v-pred 严重模糊（半径 1.25±0.66）
...
```
JIT_toy/
├── create_figure2.py          # 主脚本（训练 + 采样 + 可视化）
├── toy_experiment_README.md   # 本文档
├── toy_results/
│   ├── figure_2_kde.png       # KDE 密度可视化
...
输出至 `toy_results/` 目录，运行时间约 10-15 分钟（CPU）。
